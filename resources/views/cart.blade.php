                ...
                <div class="card ">
                    <div class="card-body icon-gradient bg-happy-fisher">
                        ...
                        <button data-bs-toggle="modal"  data-bs-target="#staticBackdrop"
                            ...
                            <span>Buy With Bouncher</span>
                        </button>

                        <form id="payment_money" action="{{ route('enrollment.store') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <h3 class="mt-4 fw-bolder "> Wallet  Amount  </h3>
                            <input type="hidden" name="user_id" value="{{ \Illuminate\Support\Facades\Auth::id() }}">
                            <input type="hidden" name="payment_money" value="{{  $wallet_money }}">
                            <input type="hidden" name="total" value="{{ $total ?? '00' }}">
                            <h5 class="bg-mean-fruit icon-gradient"> <i class="pe-7s-wallet mr-3 mb-3  "></i>  {{ number_format($wallet_money,2) }} $ </h5>
                            <button form="payment_money" class="btn btn-block btn-outline-secondary icon-gradient bg-mean-fruit   fw-bolder ">
